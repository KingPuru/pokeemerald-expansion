 ...
 *  - Advances to Task_NewGameBirchSpeech_NuzlockeHC when done.
 * 
 * Task_NewGameBirchSpeech_NuzlockeHC
 * Task_NewGameBirchSpeech_WaitToShowNuzlockeHCMenu
 * Task_NewGameBirchSpeech_ChooseNuzlockeHC
...
static void Task_NewGameBirchSpeech_NuzlockeHC(u8);
...
static void Task_NewGameBirchSpeech_WaitToShowNuzlockeHCMenu(u8);
static void Task_NewGameBirchSpeech_ChooseNuzlockeHC(u8);
...
static void NewGameBirchSpeech_ShowNuzlockeHCMenu(void);
static s8 NewGameBirchSpeech_ProcessNuzlockeHCMenuInput(void);
static void NewGameBirchSpeech_ClearNuzlockeHCWindow(u8, u8);
...
static const struct MenuAction sMenuActions_NuzlockeHC[] = {
    {COMPOUND_STRING("YES"), {NULL}},
    {COMPOUND_STRING("NO"), {NULL}}
};
...
static void Task_NewGameBirchSpeech_ChooseNuzlocke(u8 taskId)
{
    ...
    switch (nuzlocke)
    {
        case 0:
            ...
            gTasks[taskId].func = Task_NewGameBirchSpeech_NuzlockeHC;
            break;
        case 1:
            PlaySE(SE_SELECT);
...
static void Task_NewGameBirchSpeech_NuzlockeHC(u8 taskId)
{
    NewGameBirchSpeech_ClearWindow(0);
    StringExpandPlaceholders(gStringVar4, gText_Birch_NuzlockeHC);
    AddTextPrinterForMessage(TRUE);
    gTasks[taskId].func = Task_NewGameBirchSpeech_WaitToShowNuzlockeHCMenu;
}

static void Task_NewGameBirchSpeech_WaitToShowNuzlockeHCMenu(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        NewGameBirchSpeech_ShowNuzlockeHCMenu();
        gTasks[taskId].func = Task_NewGameBirchSpeech_ChooseNuzlockeHC;
    }
}

static void Task_NewGameBirchSpeech_ChooseNuzlockeHC(u8 taskId)
{
    int nuzlockeHC = NewGameBirchSpeech_ProcessNuzlockeHCMenuInput();
    switch (nuzlockeHC)
    {
        case 0:
            PlaySE(SE_SELECT);
            FlagSet(FLAG_NUZLOCKEHC);
            FlagSet(FLAG_NUZLOCKEBAN);
            NewGameBirchSpeech_ClearNuzlockeHCWindow(1, 1);
            gTasks[taskId].func = Task_NewGameBirchSpeech_WhatsYourName;
            break;
        case 1:
            PlaySE(SE_SELECT);
...
static void NewGameBirchSpeech_ShowNuzlockeHCMenu(void)
{
    DrawMainMenuWindowBorder(&sNewGameBirchSpeechTextWindows[1], 0xF3);
    FillWindowPixelBuffer(1, PIXEL_FILL(1));
    PrintMenuTable(1, ARRAY_COUNT(sMenuActions_NuzlockeHC), sMenuActions_NuzlockeHC);
    InitMenuInUpperLeftCornerNormal(1, ARRAY_COUNT(sMenuActions_NuzlockeHC), 1);
    PutWindowTilemap(1);
    CopyWindowToVram(1, COPYWIN_FULL);
}
...
static void NewGameBirchSpeech_ClearNuzlockeHCWindowTilemap(u8 bg, u8 x, u8 y, u8 width, u8 height, u8 unused)
{
    FillBgTilemapBufferRect(bg, 0, x + 255, y + 255, width + 2, height + 2, 2);
}

static void NewGameBirchSpeech_ClearNuzlockeHCWindow(u8 windowId, bool8 copyToVram)
{
    CallWindowFunction(windowId, NewGameBirchSpeech_ClearNuzlockeHCWindowTilemap);
    FillWindowPixelBuffer(windowId, PIXEL_FILL(1));
    ClearWindowTilemap(windowId);
    if (copyToVram == TRUE)
        CopyWindowToVram(windowId, COPYWIN_FULL);
